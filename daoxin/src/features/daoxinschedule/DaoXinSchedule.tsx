      ...
      [
        {
          name: 'goal',
          title: '🎯 ' + t('goal'),
          items: schedules.filter((s) => s.scheduleCategory === 'goal'),
        },
        {
          name: 'interval',
          title: '⏳ ' + t('interval'),
          items: schedules.filter((s) => s.scheduleCategory === 'interval'),
        },
        {
          name: 'periodic',
          title: '🔄 ' + t('periodic'),
          items: schedules.filter((s) => s.scheduleCategory === 'periodic'),
        },
                  ...
                  <div className='item-info'>
                    ...
                    {isGoal && (
                      <span className='item-desc'>
                        {(item.config as GoalConfig).currentCount} /{' '}
                        {(item.config as GoalConfig).targetCount}
                      </span>
                    )}
                    {isPeriodic && (
                      <span className='item-desc'>
                        {t('currentperiodic')}: {(item.config as PeriodicConfig).periodCount}
                        {t('periodiccount')}
                      </span>
                    )}
                    {isInterval &&
